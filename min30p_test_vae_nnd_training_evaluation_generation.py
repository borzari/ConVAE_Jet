    ...
    for latent_dim in latent_dim_seq:
        ...
        class ConvNet(nn.Module):
            def __init__(self):
                ...
                self.conv1 = nn.Conv2d(1, 1*n_filter, kernel_size=(num_features,5), stride=(1), padding=(0))
                self.conv2 = nn.Conv2d(1*n_filter, 2*n_filter, kernel_size=(1,5), stride=(1), padding=(0))
                self.conv3 = nn.Conv2d(2*n_filter, 4*n_filter, kernel_size=(1,5), stride=(1), padding=(0))
                #
                self.fc1 = nn.Linear(1 * int(num_particles - 12) * 4*n_filter, 1500)
                self.fc2 = nn.Linear(1500, 2 * latent_dim)
                #
                self.fc3 = nn.Linear(latent_dim, 1500)
                self.fc4 = nn.Linear(1500, 1 * int(num_particles - 12) * 4*n_filter)
                self.conv4 = nn.ConvTranspose2d(4*n_filter, 2*n_filter, kernel_size=(1,5), stride=(1), padding=(0))
                self.conv5 = nn.ConvTranspose2d(2*n_filter, 1*n_filter, kernel_size=(1,5), stride=(1), padding=(0))
                self.conv6 = nn.ConvTranspose2d(1*n_filter, 1, kernel_size=(num_features,5), stride=(1), padding=(0))
            ...
            def decode(self, z):
                ...
                out = self.fc4(out)
                out = torch.relu(out)
                out = out.view(batch_size, 4*n_filter, 1, int(num_particles - 12)) # reshaping
                out = self.conv4(out)
                out = torch.relu(out)
                out = self.conv5(out)
                out = torch.relu(out)
